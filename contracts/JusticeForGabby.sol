...
contract JusticeForGabby is Context, IERC20, Ownable {
    ...
    string private constant _name = "Justice For Gabby";
    string private constant _symbol = "Gabs";
    ...
    constructor (address payable charityAddress, address payable maintenanceAddress) {
    ...
    function setMaintenanceFeePercent(uint256 maintenanceFee) external onlyOwner() onlyUnlocked(){
        require(maintenanceFee <= 50, "Cannot set percentage over 0.50%");
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ValuesResult memory valuesResult = ValuesResult(0, 0, 0, 0, 0, 0, 0, 0, 0);

        _getTValues(tAmount, valuesResult);
        _getRValues(tAmount, valuesResult, _getRate());

        return (valuesResult.rAmount, valuesResult.rTransferAmount, valuesResult.rFee, valuesResult.tTransferAmount, valuesResult.tFee, valuesResult.tCharity, valuesResult.tMaintenance);
    }
    ...
    function _getTValues(uint256 tAmount, ValuesResult memory valuesResult) private view returns (ValuesResult memory) {
        ...
        valuesResult.tTransferAmount = tAmount.sub(valuesResult.tFee).sub(valuesResult.tCharity).sub(valuesResult.tMaintenance);
        return valuesResult;
    }
    ...
    function _getRValues(uint256 tAmount, ValuesResult memory valuesResult, uint256 currentRate) private pure returns (ValuesResult memory) {
        ...
        valuesResult.rTransferAmount = valuesResult.rAmount.sub(valuesResult.rFee).sub(valuesResult.rCharity).sub(valuesResult.rMaintenance);
        return (valuesResult);
    }
    ...
    function addTokensTransferred(address wallet, uint256 amount) private {
        uint256 rate = _taxFee.add(_charityFee).add(_maintenanceFee);
    ...
    */
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tCharity, uint256 tMaintenance) = _getValues(tAmount);
